...
Fields are used for validation of data on model level

```
from typing import Annotated
from fasapi import FastAPI, Body
...
class Item(BaseModel):
    ...
    description: str | None = Field(
        default = None, title = "The description of the field", 
        max_length = 300
    )
    price: float = Feild(gt=0, description="The price must be greateer than 0")
    tax: float | None = None



@app.put("/items/{item_id}")
async def update_item(item_id: int, item: Anootated[Item, Body(embed=True)]):
